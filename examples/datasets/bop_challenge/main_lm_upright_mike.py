...
target_bop_objs = bproc.loader.load_bop_objs(bop_dataset_path = os.path.join(args.bop_parent_path, 'lm'), mm2m = False)
...
def sample_apple_center(obj: bproc.types.MeshObject):
    # 蘋果專屬，小範圍擺放
    min = np.array([-0.03, -0.03, 0.02])
    max = np.array([0.03, 0.03, 0.05])
    obj.set_location(np.random.uniform(min, max))
    obj.set_rotation_euler(bproc.sampler.uniformSO3())

def sample_pose_func(obj: bproc.types.MeshObject):
    # 亂丟，不在中央
    min = np.array([-0.3, -0.3, 0.0])
    max = np.array([0.3, 0.3, 0.2])
    obj.set_location(np.random.uniform(min, max))
    obj.set_rotation_euler(bproc.sampler.uniformSO3())
...
for i in range(args.num_scenes):
    ...
    # 只挑選 category_id 為 15 的物件
    apple_objs = []
    for obj in sampled_target_bop_objs:
        try:
            if obj.get_cp("category_id") == 15:
                apple_objs.append(obj)
        except Exception as e:
            pass  # 如果沒這個屬性就跳過
    ...
    if len(apple_objs) > 0:
        ...
        print("Warning: No apple (category_id=15) in this scene.")
